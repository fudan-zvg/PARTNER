...
from det3d.core.utils.center_utils import bbox3d_overlaps_iou, bbox3d_overlaps_giou, bbox3d_overlaps_diou
...
class IOULoss(nn.Module):
    ...
    def forward(self, pred_boxes, pred_ious, gt_boxes):
        target_ious = boxes_iou3d_gpu(pred_boxes, gt_boxes)[range(pred_boxes.shape[0]), range(gt_boxes.shape[0])]
        target_ious[torch.isnan(target_ious)] = 0
        target_ious = 2 * target_ious - 1
        iou_loss = self.crit_iou(pred_ious.squeeze(-1), target_ious)
        return iou_loss.sum()
...
class IouRegLoss(nn.Module):
    ...
    def forward(self, box_pred, box_gt):
        iou = self.bbox3d_iou_func(box_pred, box_gt)
        loss = (1. - iou).sum()
        return loss
